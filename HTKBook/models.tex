...
to one of the following categories (called the \textit{HSKind}\index{hskind@\textit{HSKind}}
\begin{itemize}
\item \texttt{PLAINHS}
\item \texttt{SHAREDHS}
\item \texttt{TIEDHS}
...
\mysect{Regression Class Trees for Adaptation}{regtreemods}

In order to perform adaptation \HTK\ generally \index{adaptation!regression tree}requires the use of a
...
\htool{HHEd} (see section~\ref{s:hhedregtree}).
