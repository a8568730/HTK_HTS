...
\htool{HERest} using maximum likelihood linear regression
...
adaptation is provided by \htool{HVite}, using just MLLR.
...
regression class tree and store it along with the set of baseclasses. For
example,
...
\htool{HERest} can be used to perform static adaptation, where all the
adaptation data is processed in a single block. \htool{HVite} can be used
...
forms can be advantageous if storage is an issue since the TMFs are
significantly smaller than MMFs and the computational overhead
incurred when transforming a model set using a TMF is negligible.
...
\begin{verbatim}
    HERest -C config -C config.global -S adapt.scp -I adaptPhones.mlf -H hmm16/macros -u a\
            -H hmm16/hmmdefs -z global.tmf  -h '*/%%%%%%_*.mfc' tiedlist

    HERest -a -C config -C confi.rc -S adapt.scp -I adaptPhones.mlf -H hmm16/macros -u a\ 
            -H hmm16/hmmdefs -H global.tmf -z rc.tmf  -h '*/%%%%%%_*.mfc' tiedlist
\end{verbatim}
...
\end{verbatim}
\texttt{-h} is a mask that is used to detect when the speaker changes and also
is used to determine the name of the speaker transform.
...
\begin{verbatim}
    ...
    HVite -H hmm16/macros -H hmm16/hmmdefs -S testAdapt.scp -l '*' \ 
          -H rc.tmf   -h '*/%%%%%%_*.mfc' -k  -i recoutAdapt.mlf -w wdnet \ 
          -p 0.0 -s 5.0 dict tiedlist
